...
test_that('methods work',{
  ...
  expect_false(FALSE %in% sapply(m,function(x){identical(class(x %>% dat()),c('tbl_df',"tbl","data.frame"))}))
  expect_false(FALSE %in% sapply(m,function(x){identical(class(x %>% sinfo()),c('tbl_df',"tbl","data.frame"))}))
  expect_false(FALSE %in% sapply(m,function(x,col){ncol(dat(x)) == col},col = ncol(dat(d))))
  expect_false(FALSE %in% sapply(m,function(x,row){nrow(dat(x)) == row},row = nrow(dat(d))))
  expect_false(FALSE %in% sapply(m,function(x,col){ncol(sinfo(x)) == col},col = ncol(sinfo(d))))
  expect_false(FALSE %in% sapply(m,function(x,row){nrow(sinfo(x)) == row},row = nrow(dat(d))))
})