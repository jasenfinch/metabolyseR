...
test_that('unsupervised random forest works',{
  rf <- randomForest(d,cls = NULL,nCores = 2,returnModels = TRUE)
...
test_that('random forest regression works',{
  rf <- randomForest(d,cls = 'injorder',perm = 3,nCores = 2,returnModels = TRUE)
...
test_that('low sample permutation testing works',{
  rf <- analysisData(data = abr1$neg,info = abr1$fact) %>%
