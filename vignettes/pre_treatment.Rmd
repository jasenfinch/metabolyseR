...
There will be examples of their application during exploratory analyses along with useful visualisations that can aid interpretation of when particular treatments should be applied as well as their effect once they have been used. 
...
The alternative strategy is by applying a minimum threshold; where the minimum occupancy across all classes is required to be above the freshold.
...
`metabolyseR` provides methods for mean, median and sum aggregating data and each starts with the `aggregate` prefix.

Below shows a principle component analysis (PCA) plot of the example data coloured by the classes of `day` sample information column.
It is first maximum occupancy filtered to remove empty columns.
...
With appropriate sample randomisation (see section Feature filtering based on QC samples) batch related variability can be corrected for using an average centring correction method on the individual features.

The plot below shows differences in the total ion count distributions for each of the classes in the `day` sample information column.
...
The example data set does not include QC samples so for this example the `H` class will be used.
...
`metabolyseR` contains a number of methods for applying pre-treatment routines specifically on QC samples.
These are all prefixed with `QC` and include feature filtering of a data set based the occupancy of the QC class, imputation of the QC class only, feature filtering based in the RSD values of the QC class and removal of only the QC class.
...
The default pre-treatment parameters can first assigned to the variable `p`.
