...
tableGenerate <- function(method){
  desc <- method(description = TRUE)
  ...
  meth <- names(desc)
  defaults <- lapply(method(),formals)
  defaults <- lapply(defaults,function(x){x$d <- NULL;return(x)})
  defaults <- lapply(defaults,function(x){tibble(Parameter = names(x),Value = unlist(x))})
  ...
  tab <- tibble(Method = meth, Description = desc, Arguments = args) %>%
    left_join(tibble(Method = names(defaults),Defaults = defaults),
              by = 'Method')
  return(tab)
}
```
...
tab <- tableGenerate(metabolyseR:::removeMethods)
kable(tab)
```
...
tab <- tableGenerate(metabolyseR:::keepMethods)
kable(tab)
```
...
tab <- tableGenerate(metabolyseR:::occupancyMethods)
kable(tab)
```
...
tab <- tableGenerate(metabolyseR:::correctionMethods)
kable(tab)
```
...
tab <- tableGenerate(metabolyseR:::aggregateMethods)
kable(tab)
```
...
tab <- tableGenerate(metabolyseR:::imputeMethods)
kable(tab)
```
...
tab <- tableGenerate(metabolyseR:::transformMethods)
kable(tab)
```
...
tab <- tableGenerate(metabolyseR:::QCMethods)
kable(tab)
```
