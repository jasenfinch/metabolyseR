...
The examples shown here use the `abr1` data set from the *[FIEmspro](https://github.com/wilsontom/FIEmspro)* package (`?FIEmspro::abr1`).
...
parameters <- analysisParameters()
parameters
parameters <- changeParameter('clusterType','PSOCK',parameters)
parameters
```
...
Binary class comparisons of all class combinations are run separately around which the method is parallised.
...
* `pairwises` - Optional vector of specific pairwise comparisons to analyse in the form of `'Class1~Class2'`.
...
The classification results will then be returned as a table containing the accuracy, AUCs and margins for each iteration, method and pairwise.

## Feature Selection

Where discrimination exists between sample classes it is important to identify the variables (features) that are responsible for this.
...
The results will be returned as a table containing the scores and *p* values for each variable, pairwise and method.
...
args <- unlist(args)
score <- unlist(lapply(desc,function(x){return(x$score)}))
Pvalue <- unlist(lapply(desc,function(x){return(x$Pvalue)}))
type <- unlist(lapply(desc,function(x){return(x$type)}))
desc <- unlist(lapply(desc,function(x){return(x$description)}))
...
defaults <- unlist(defaults)
tab <- tibble::tibble(Method = meth, Description = desc, Arguments = args, Defaults = defaults, Score = score,`p value` = Pvalue,Type = type)
...
* classification
    + `plotClassification()` - classification measures of pairwise comparisons
    + `plotClassificationDendrogram()` - dendrogram of classification results

* feature selection
    + `plotFeatureSelecton()` - feature selection scores of pairwise comparisons
