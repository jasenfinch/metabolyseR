...
This provides a set of technical injections that allows the variablity in instrument performance over the run to be accounted for and the robustness of the acquired variables to be assessed.
...
This variable filtering strategy has an advantage over that of occupancy alone as it is not dependent on underlying class struture.
...
Prior to downstream analyses, metabolomics data often require a transfromation step to allow the data to meet the assumptions of a particular statistical inference or account for potential variablility in sample concentration.
...
The classification results will then be returned within the classification slot of the Analysis object as an object of class `accest` (`?FIEmspro::accest`).
