...
Parameter selection is the fundamental aspect for performing routine analyses *metabolyseR* and will be the step requiring the most input from the user.
...
parameters <- analysisParameters(c('pre-treatment','modelling'))
parameters
```
...
parameters <- changeParameter(parameters,'cls','class',elements = 'pre-treatment')
```
